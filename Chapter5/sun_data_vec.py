...
def main():
    ...
    monthly_mean = np.zeros(12) # list with 12 elements
    ...
    ind, = np.where(np.isclose(monthly_mean, max_value))

    month = month_names[ind[0]]
    print('% s has best weather with % .1f sun hours on average' % \
            (month, max_value))

    decade_mean = np.array([])
    for decade_start in range(1930, 2010, 10):
        ...
        decade_mean=np.append(decade_mean,s/(20.*30))
    for i in range(decade_mean.size):
